...
\subsection{Metrics}
Model performance was evaluated using a hold-out test set to ensure generalisability, with predictive accuracy quantified through Mean Absolute Percentage Error (MAPE) and Root Mean Square Error (RMSE) to penalise significant forecasting deviations during peak periods. Adjusted $R^2$ was utilised to measure the proportion of variance explained while accounting for the increased complexity of the multi-feature feature set. The following \autoref{tab:metrics_desc} below illustrates the chosen metrics.
...
\subsubsection{Does temperature have a non-linear relationship with NSW grid demand across 2°C bins?}
...
\subsubsection{Do mean hourly demand profiles differ between weekdays, weekends, and public holidays?}
